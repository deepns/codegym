...
namespace
{
    ...
    void ShowHelp()
    {
        std::cout << "./main -r/--required <arg> -o/--optional [arg] -h/--help";
    }
    ...
    {
        // With long options, define the option's long form (and the associated
        // short-form) in an array of type 'option'
        // It is important to have a NULL value in the option array so getopt_log
        // can stop parsing.

        option long_options[] = {
            {"required", required_argument, nullptr, 'r'},
            {"optional", optional_argument, nullptr, 'o'},
        ...
        int ch = -1;
        while ((ch = getopt_long(argc, argv, short_options, long_options, nullptr /* long_index */)) != -1)
        {
            switch (ch)
            {
            case 'r':
